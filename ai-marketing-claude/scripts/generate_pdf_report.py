...
# Register Unicode-compatible TTF fonts for French accented characters
_FONT_REGULAR = "Arial"
...
try:
    ...
    # Fallback: try common Linux/Mac paths or use Helvetica
    try:
        pdfmetrics.registerFont(TTFont(_FONT_REGULAR, "/usr/share/fonts/truetype/dejavu/DejaVuSans.ttf"))
...
def draw_score_gauge(score, x, y, size=80):
    ...
    d.add(String(size / 2 + 10, size / 2 + 10, str(int(score)),
                 ...
                 textAnchor="middle", fontName=_FONT_BOLD))
...
def create_bar_chart(categories, scores, width=450, height=180):
    ...
    for i, (cat, score) in enumerate(zip(categories, scores)):
        ...
        d.add(String(label_x, y + 5, cat[:22],
                     ...
                     textAnchor="start", fontName=_FONT_REGULAR))
        ...
        d.add(String(bar_x + max_bar_width + 10, y + 5, f"{int(score)}",
                     ...
                     textAnchor="start", fontName=_FONT_BOLD))
...
def generate_report(data, output_path):
    ...
    for i, f in enumerate(findings, 1):
        ...
        table_style_cmds.append(("FONTNAME", (0, i), (0, i), _FONT_BOLD))
